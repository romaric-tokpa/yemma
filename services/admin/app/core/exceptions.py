...
class AdminError(Exception):
    """Exception de base pour les erreurs admin"""
...
class CandidateNotFoundError(AdminError):
    """Candidat non trouvé"""
    def __init__(self, candidate_id: str):
        super().__init__(f"Candidate with id {candidate_id} not found", status.HTTP_404_NOT_FOUND)


async def admin_error_handler(request: Request, exc: AdminError):
    """Handler pour les erreurs admin"""
    return JSONResponse(

      ...
      <div className="w-full min-w-0 px-3 xs:px-4 sm:px-6 lg:px-8 py-4 lg:py-6">
        ...
        <div className="flex items-start gap-2 sm:gap-3 mb-4">
          <Link
            ...
            className="shrink-0 p-1.5 rounded-lg text-[#6b7280] hover:bg-[#E8F4F3] hover:text-[#226D68] transition-colors mt-0.5"
        ...
        <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
          {applications.length > 0 && (
            <div className="px-3 sm:px-4 py-2 border-b border-gray-100 bg-[#F8FAFC]/60 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2 sm:gap-3">
              <div className="relative flex-1 min-w-0 sm:min-w-[160px] sm:max-w-xs">
                ...
                <Input
                  ...
                  className="h-8 pl-8 text-xs border-gray-200 focus:ring-1 focus:ring-[#226D68]/30 w-full"
                />
              </div>
              <div className="flex items-center gap-2">
                <select
                  ...
                  className="h-8 px-2 sm:px-3 rounded-lg border border-gray-200 bg-white text-xs text-[#2C2C2C] focus:border-[#226D68] focus:ring-1 focus:ring-[#226D68]/30 focus:outline-none flex-1 sm:flex-none min-w-0"
                >
                ...
                {(search || statusFilter) && (
                  <button
                    ...
                    className="h-8 px-2.5 rounded-lg text-xs text-[#6b7280] hover:bg-gray-100 hover:text-[#226D68] transition-colors shrink-0"
                  >
                    Réinitialiser
                  </button>
                )}
            ...
            <>
              ...
              <div className="sm:hidden divide-y divide-gray-100">
                {filteredApplications.map((app) => {
                  ...
                  return (
                    <div key={app.id} className="px-3 py-3 space-y-2.5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full overflow-hidden ring-2 ring-gray-100 shrink-0">
                          <img
                            src={displayPhoto}
                        ...
                        <Link to={ROUTES.ADMIN_REVIEW(app.candidate_id)} className="shrink-0">
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-7 px-2 text-xs text-[#226D68] hover:bg-[#E8F4F3]"
                          >
                      ...
                      {app.profile_title && (
                        <p className="text-xs text-[#226D68] truncate">{app.profile_title}</p>
                      )}
                      <div className="flex items-center gap-2">
                        <select
                          ...
                          className={`h-7 flex-1 min-w-0 px-2 rounded-lg text-xs font-medium border cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#226D68]/30 ${
                            app.status === 'HIRED'
                        ...
                        {updatingStatusId === app.id && (
                          <Loader2 className="h-3.5 w-3.5 animate-spin text-[#226D68] shrink-0" />
                        )}
        ...
        <Dialog open={!!rejectDialog} onOpenChange={(open) => !open && setRejectDialog(null)}>
          <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-md p-4 sm:p-6">
            <DialogHeader>
              <DialogTitle className="text-base sm:text-lg">Motifs de refus</DialogTitle>
              <DialogDescription className="text-xs sm:text-sm text-[#6b7280]">
            ...
            <Textarea
              ...
              className="min-h-[80px] sm:min-h-[100px] mt-2 text-sm"
              maxLength={2000}
            />
            <DialogFooter className="flex-col-reverse sm:flex-row gap-2 sm:gap-0">
              <Button variant="outline" size="sm" onClick={() => { setRejectDialog(null); setRejectReason('') }} className="w-full sm:w-auto">
                Annuler
              </Button>
              <Button size="sm" onClick={handleRejectConfirm} disabled={!rejectReason?.trim()} className="w-full sm:w-auto">
                Confirmer le refus
              </Button>
            </DialogFooter>
          </DialogContent>
